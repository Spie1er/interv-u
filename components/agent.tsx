...
const Agent = ({
  ...
  useEffect(() => {
    if (callStatus === CallStatuses.FINISHED) {
      if (type === 'generate') {
        ...
        handleGenerateFeedback(messages).then() //TODO Check if then is needed
      }
    }
  ...
  return (
    <>
      {/*INTERVIEWER AND INTERVIEWEE CARDS*/}
      <div className='call-view'>
        ...
        <div className='card-border'>
          <div className='card-content'>
            <div className='avatar'>
              <Image
                src={`https://api.dicebear.com/9.x/bottts/png?seed=${encodeURIComponent(
                  userName || 'default'
                )}`}
                alt='user avatar'
                width={540}
                height={540}
                className='rounded-full object-cover size-[120px] p-5'
              />
            </div>
            <h3>{userName}</h3>
          </div>
